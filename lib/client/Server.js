...
class Server extends Server_1.default {
    ...
    setSplit_limitAmount(amount) {
        this.featureLimits.split_limit = amount;
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                let res = yield this.api.call(`/application/servers/${this.id}/build`, 'PATCH', this.getBuildRequestObject({ feature_limits: { split_limit: amount } }));
                resolve(new Server(this.api, res.data.attributes));
            }
            catch (error) {
                reject(error);
